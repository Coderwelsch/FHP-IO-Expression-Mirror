...
for x in participants:
	...
	for sessions in glob.glob("%s/*" %x): #Store list of sessions for current participant
		for files in glob.glob("%s/*" %sessions):
			...
			# if dest_neut path directories not created yet
			if not os.path.exists(os.path.dirname(dest_emot)):
				os.makedirs(os.path.dirname(dest_emot))

			# if dest_emot path directories not created yet
			if not os.path.exists(os.path.dirname(dest_emot)):
				os.makedirs(os.path.dirname(dest_emot))

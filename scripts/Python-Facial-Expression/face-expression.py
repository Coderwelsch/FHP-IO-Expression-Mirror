...
import glob
import os
from lib import UpdateModel

video_capture = cv2.VideoCapture(0)
facecascade = cv2.CascadeClassifier("cascades/haarcascade_frontalface_default.xml")
fishface = cv2.face.createFisherFaceRecognizer()
try:
	fishface.load("cascades/trained_emoclassifier.xml")
except:
...
while True:
	...
	if args.update: #If update flag is present, call update function
		...
		break
	elif len(facedict) == 5: #otherwise it's regular a runtime, continue normally with emotion detection functionality
		recognize_emotion()
		facedict = {}
		# break